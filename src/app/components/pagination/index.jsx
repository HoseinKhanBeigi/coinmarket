...
const Pagination = ({
  ...
  const [showPaginationState, setShowPaginationState] = useState([
    1, 2, 3, 4, 5,
  ]);
  ...
  for (let i = 1; i <= Math.ceil(getData / dataPerPageState); i++) {
    pageNumbers.push(i);
  }
  ...
  const NewCurrentPageData = new Array(5000)
    .fill(0)
    .slice(indexOfFirstPost, indexOfLastPost);

  useEffect(() => {
    if (pageNumbers.length > 10) {
      setNewPageData([6, 7, 8, 9, 10]);
    }
